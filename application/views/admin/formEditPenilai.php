...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0">Edit Tim Penilai</h1>
				</div><!-- /.col -->
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="#">Home</a></li>
						<li class="breadcrumb-item active">Edit Tim Penilai</li>
					</ol>
				</div><!-- /.col -->
			</div><!-- /.row -->
		</div><!-- /.container-fluid -->
	...
	<section class="content">
		<div class="container-fluid">
			...
			<div class="row">
				<div class="col-12">
					<div class="card">
						<form action="<?= base_url() . 'C_Admin/updatePenilai' ?>" method="post"
							id="formEditTimPenilai">
							<div class="card-header">
								<h3 class="card-title">Edit Data Tim Penilai</h3>
							</div>
							<!-- /.card-header -->
							<div class="card-body">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="password" id="password"
										maxlength="255" required minlength="8" pattern="\S(.*\S)?"
										value="<?= $penilai['password'] ?>">
									<small class="text-danger" id="password_error"></small>
								</div>
								<div class="form-group">
									<label for="nama">Nama Tim Penilai <span class="text-danger">*</span></label>
								...
								<div class="form-group">
									...
									<input type="number" class="form-control" name="no_telp" id="no_telp"
										...
										value="<?= $penilai['no_telp'] ?>">
									<small class=" text-danger" id="no_telp_error"></small>
								</div>
							</div>
							<input type="hidden" name="id_user" id="id_user" value="<?= $penilai['id_user'] ?>"></div>
...
<script>
	...
	const inputFields = ['username', 'password', 'nama', 'no_telp'];
	...
	function batal(event) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				document.location.href = '<?= base_url() . 'C_Admin/penilai' ?>';
			}
		})
	};
	...
	function resetForm(event) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				document.getElementById('formEditTimPenilai').reset();
			}
		})
	};
	...
	function simpan(event) {
		...
		if (!nama) {
			document.getElementById('nama_error').innerText = 'Nama Tim Penilai tidak boleh kosong';
			return;
		}
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				document.getElementById('formEditTimPenilai').submit();
			}
		})
	};
