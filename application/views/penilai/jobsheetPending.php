	...
	<section class="content">
		<div class="container-fluid">
			...
			<div class="row">
				...
				<div class="col-12">
					<div class="mb-4">
						<a href="<?= base_url() . 'C_Penilai/formTambahJobsheet?from=pending' ?>">
					...
					<div class="card">
						...
						<div class="card-body">
							<?php if (empty($jobsheet)): ?>
								<div role="alert">
									Data jobsheet pending masih kosong,
									cobalah klik tambah data jobsheet untuk menambahkan jobsheet
								</div>
							<?php else: ?>
									...
									<tbody>
										<?php foreach ($jobsheet as $j): ?>
											<tr>
												...
												<td><?= $j['nama'] ?></td>
												<td>
													<a href="<?= base_url() . 'C_Penilai/formEditJobsheet/' . $j['id_jobsheet'] ?>?from=pending"
 ...
 if (urlParams.get('status') === 'updated') {
  Swal.fire({
   ...
   window.history.replaceState({ path: newUrl }, '', newUrl);
	...
	if (urlParams.get('status') === 'revisi') {
		Swal.fire({
			icon: 'success',
			title: 'Berhasil',
			text: 'Jobsheet berhasil direvisi',
		}).then(() => {
			window.history.replaceState({ path: newUrl }, '', newUrl);
		});
	}
 ...
 function hapus(id) {
  Swal.fire({
   ...
   if (result.isConfirmed) {
    window.location.href = "<?= base_url() . 'C_Penilai/hapusJobsheet/' ?>" + id + '?from=pending';
   }
  });
 }
</script>
